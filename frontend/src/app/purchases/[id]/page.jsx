      ...
      {[1, 2, 3, 4, 5].map((n) => (
        ...
        >
          <span
            style={{
              fontSize: 28,
              color: n <= (hover || value || 0) ? "#fbbf24" : "var(--border)",
            }}
          >
            ★
          </span>
...
function generateReceiptHtml(order) {
  ...
  const itemRows = (order.order_items || [])
    .map((oi) => {
      const title =
        oi.item_title || oi.item_snapshot?.title || `Item #${oi.item_id}`;
...
export default function PurchaseDetailPage() {
  ...
  if (!order) {
    return (
      <div className="mx-auto max-w-2xl px-6 py-16">
        ...
        <Link href="/purchases" className="btn-forge inline-flex mt-4">
          Back
        </Link>
      </div>
    );
  ...
  const canDispute = ["confirmed", "shipped", "delivered"].includes(
    order.status,
  );
  const addr = order.shipping_address_details;
  const addrText = addr
      ...
      {(order.order_items || []).map((oi) => {
        ...
        return (
          ...
          >
              ...
              <div className="flex-1 min-w-0">
                ...
                >
                  ...
                  {snap.shipping_mode && (
                    ...
                    >
                      {shippingModeMap[snap.shipping_mode] ||
                        snap.shipping_mode}
                    </span>
                  )}
                  {snap.bundle_name && (
                    <span
                      style={{
                        ...
                        border:
                          "1px solid var(--primary-border-soft, var(--border))",
                        borderRadius: 4,
                        padding: "1px 6px",
                        color: "var(--primary)",
                      }}
            ...
            {/* Selected options */}
            {oi.selected_options &&
              Object.keys(oi.selected_options).length > 0 && (
            ...
            <div>
              ...
              {shipping ? (
                ...
                >
                  {shipping.method && (
                    <p>Method: {capitalize(shipping.method)}</p>
                  )}
                  {shipping.tracking_number ? (
                    ...
                    <p style={{ color: "var(--text-muted)" }}>
                      Tracking will appear when the seller ships.
                    </p>
                  )}
                  {shipping.estimated_days && (
                    <p style={{ color: "var(--text-muted)" }}>
                      Est. {shipping.estimated_days} day
                      {shipping.estimated_days !== 1 ? "s" : ""}
                    </p>
                  )}
                ...
                <p className="text-sm" style={{ color: "var(--text-muted)" }}>
                  Shipping details will appear once confirmed.
                </p>
              )}
            </div>
                  ...
                  <div className="space-y-3">
                    ...
                    <textarea
                      ...
                      onChange={(e) =>
                        setReviewState((prev) => ({
                          ...prev,
                          [oi.id]: { ...prev[oi.id], body: e.target.value },
                        }))
                      }
                      rows={2}
